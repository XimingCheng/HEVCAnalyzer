...
public:
    ...
    : m_sYUVPath(path), m_iSourceWidth(w), m_iSourceHeight(h), m_iYUVBit(bit), m_pImageList(pImageList), m_pFrame(pFrame),
      m_iFrameNumbers(framenumbers), m_pcPicYuvOrg(NULL), m_bAdaptiveFrameNumners(true)
    { }
    ~ThumbnailThread();
    void setAdaptiveFrameNumners(bool b) { m_bAdaptiveFrameNumners = b; }
    bool getAdaptiveFrameNumners() const { return m_bAdaptiveFrameNumners; }
